    ...
    this.router.events
      ...
      .subscribe((event) => {
        ...
        if (event instanceof NavigationEnd) {
          ...
          let position = event.url.indexOf('=') ;
          let urlParams = position >= 0 ? event.url.substring(position + 1) : '';

          //Si la url es diferente a la del buscador y el campo de busqueda no esta vacio, lo reseteo
          if (
            !urlActualAComparar.includes(urlBuscador) &&
            this.searchFieldValue !== ''
          ) {
            console.log('Resetear input');
            this.searchFieldValue = '';
          }
          //Si la url es la del buscador y el buscador está vacio, settear el valor de la url en el buscador
          else if (
            urlActualAComparar.includes(urlBuscador) 
            //&& this.searchFieldValue === ''
          ) {
            console.log('Poner los parametros: ' + urlParams);
            this.searchFieldValue = urlParams;
          }
        }
      });
  }
